...
function valueAt(series: number[], index: number, offset = 0): number {
  ...
  if (target < 0 || target >= series.length) return NaN
...
function resolveLevelValue(ref: LevelRef, index: number, ctx: CandleContext, cache: IndicatorSeries): number {
  ...
  switch (ref.type) {
    ...
    case 'session_high': {
      let max = -Infinity
      for (let i = 0; i <= index; i++) max = Math.max(max, ctx.high[i]!)
      return max
    }
    case 'session_low': {
      let min = Infinity
      for (let i = 0; i <= index; i++) min = Math.min(min, ctx.low[i]!)
      return min
    }
    default:
      return NaN
...
function compareValues(left: number, op: Op, right: number): boolean {
  ...
  switch (op) {
    ...
    case 'eq_within': return Math.abs(left - right) <= EQ_WITHIN_EPSILON
...
function evaluateCondition(
  ...
  switch (condition.type) {
    ...
    case 'crossover': {
      ...
      if (![aNow, bNow, aPrev, bPrev].every(Number.isFinite)) return false
...
/**
 * Evaluates a compiled condition against a candle context, testing the last candle index.
 * Returns true if the condition fires on the most recent bar.
 */
export function evaluateCompiledCondition(
  compiled: CompiledCondition,
  ctx: CandleContext,
): boolean {
  if (ctx.close.length === 0) return false
  const cache: IndicatorSeries = new Map()
  const lastIndex = ctx.close.length - 1
  return evaluateCondition(compiled.root, lastIndex, ctx, cache)
}

/**
 * Evaluates a raw Condition at a specific bar index within a candle context.
 * Useful for scanning across all bars.
 */
export function evaluateConditionAtBar(
  condition: Condition,
  index: number,
  ctx: CandleContext,
  cache?: IndicatorSeries,
): boolean {
  const indicatorCache = cache ?? new Map()
  return evaluateCondition(condition, index, ctx, indicatorCache)
}
